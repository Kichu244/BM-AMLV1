...
if (conf.LANG == 'TR') {
    ...
    already_on = 'BOTMWOL yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'BOTMWOL yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'BOTMWOL, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'BOTMWOL, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Turns on AI powered chatbot on to your account!'
    already_on = 'BOTMWOL chatbot is already on.'
    already_off = 'BOTMWOL chatbot is currently turned off!.'
    succ_on = 'BOTMWOL chatbot on! Restarting to make chatbot ✅'
    succ_off = 'BOTMWOL chatbot off Restarting to make normal  ❤️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ BOTMWOL chatbot സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'BOTMWOL പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'BOTMWOL സെമി-ഫങ്ഷണൽ ആയി സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

New.addCommand({ pattern: 'botmwol ?(.*)', desc: _dsc, fromMe: true,dontAddCommandList: true, usage: '.botmwol on / off' }, (async (message, match) => {
